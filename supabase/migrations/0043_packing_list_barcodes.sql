-- Barcodes on packing lists: a scanned catalog barcode now lands on the packing-list line
-- for that product + calidad instead of creating a separate "added during count" line.
create or replace function public.norm_product_name(t text) returns text
language sql immutable as $f$
  select btrim(regexp_replace(upper(translate(coalesce(t, ''), 'ÁÉÍÓÚÜÑİáéíóúüñı', 'AEIOUUNIaeiouuniI')), '[[:space:]]+', ' ', 'g'))
$f$;
