-- Per-product notes on a sale: alongside the sale's own general note
-- (sales.notes), each line item can now carry its own short note (e.g. a
-- specific instruction or detail about that particular product).

alter table public.sale_items add column notes text;
...
-- return_sale_item: carry the item's own note over to the "kept" row on a
-- partial return, same as custom_name already does.
create or replace function public.return_sale_item(
  p_sale_item_id uuid,
  p_quantity integer,
  p_reason text,
  p_refund_method text default 'efectivo'
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if p_quantity = v_item.quantity then
    ...
    insert into public.sale_items (sale_id, variant_id, custom_name, quantity, original_price, sold_price, cost, line_total, status, notes)
    values (
      ...
      round(v_item.sold_price * (v_item.quantity - p_quantity)), 'active', v_item.notes
    )
