-- 0029 set search_path = public, but crypt()/gen_salt() live in the
-- extensions schema on this project (pgcrypto), not public — every manual
-- SQL insert this session worked because it ran without that restricted
-- search_path. Schema-qualify explicitly rather than widening the
-- search_path, consistent with keeping security-definer functions strict.
